    ...
    if request.method == 'POST':
        ...
        if mensajes == []:
            ...
            if rsp:#la llave privada es correcta
                ...
                nombre_unico = str(uuid.uuid4())##crear un nombre unico para el hash de la firma
                ruta=os.path.join("firmas/"+nombre_unico)
                docNew.save(ruta+".pdf", pretty=True)
                ...
                cur.execute("UPDATE usuarios_documentos SET hash= '" + nombre_unico+"', estado='1' WHERE idUsuario = "+str(session['id'])+" AND idDocumento="+id)
...
def guardar_documento():
    if request.method == 'POST':
        ...
        Documento.usuarios= request.form['usuario']
